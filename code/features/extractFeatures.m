...
% regions must be relative to the clip start from which the spectrum was taken
...
%
% RY modified to stabilize custom function calls (custom functions moved to end & found w/ localfunctions);
% checked that results were the same.
...
% a trick to tell MATLAB these are nested functions - old, unstable way; RY modified
...
%fhs = cellfun(@str2func, fxnNames, 'UniformOutput',false); %apply @strfunc to all fxnNames, i.e. convert each of fxnNames strings to functions (ex. extractFeatures/attack calls function attack() in this script)

customfh = localfunctions; %list of custom local functions

fxnNames = suffixes(1:4); % matlab-native functions
fhs = [cellfun(@str2func, fxnNames, 'UniformOutput',false), customfh']; %transform each of fxnNames to functions, + local custom functions -> list of all function handles
    ...
    if numel(spectrum.times) > 0
        ...
        for jj = 1:numel(fields)
            ...
            for kk = 1:numel(suffixes)  % apply each reduction to the data
                features(ii).([fields{jj} '_' suffixes{kk}]) = fhs{kk}(featData); %apply each function to feature data
            end
        end
    else
        for jj = 1:numel(fields)
...
% define the custom functions for attack, sustain and release (note: we require no NaNs here)
