...
isObjGood     = [allNeuronCorrData.clusterObjQ] < 1.5; % < 1; JMA had it at 0.8
%%
...
for mm = 1: length(uUCSessions)
    ...
    for nn = 1: length(neuronsHere) %for each unit
        ...
        compiledNeuron.tutor_DistanceAll = horzcat([currentNeuronPairs.tutor_DistanceAll]); %compiles all tut distance measures altogether, across syllables
        %compiledNeuron.consensus_DistanceAll = horzcat([currentNeuronPairs.consensus_DistanceAll]); %RY commented out
        %compiledNeuron.central_DistanceAll = horzcat([currentNeuronPairs.central_DistanceAll]); %RY commented out
