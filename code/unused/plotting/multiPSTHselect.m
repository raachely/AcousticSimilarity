...
for ii = 1:numel(uSessions) % loop through sessions
    ...
    searchFile = [clustDir 'altClustDataAge-' num2str(birdAge) '*.mat'];
    [clustfile, file_exist] = getLatestFile([clustDir 'altClustDataAge-' num2str(birdAge) '*.mat']);
    if file_exist
        load(clustfile, 'distMats');
    else
        error('compareTutorJuvenile:distanceFileNotFound',...
            'altClustDataAge file %s for bird %s, age %d, not found', searchFile, birdID, birdAge);
    end
    clusterMat = distMats.cosim;
    centralOrder = findMostCentral(clusterMat, clusterIdxs); %cell array of syllable indices (I think); split by syllable cluster type & ordered, w/in each cluster, by some.. central order
...
%     fprintf('Central-peripheral order found.\n');
    ...
    for jj = 1:numel(spikes)
        ...
        for kk = 1:nTypes
            ...
            set(gca,'XTickLabel', num2str(str2double(get(gca,'XTickLabel')) * 1000 - preMotorT));
            set(gca,'TickDir','out');
