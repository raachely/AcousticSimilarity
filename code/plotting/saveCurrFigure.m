function fname = saveCurrFigure(fname)
    ...
    if ~contains(fname, filesep) && ~contains(fname, '/') %if path is not provided
    ...
    set(gcf, 'Position', [1 1 scrsz(3) scrsz(4)]); %maximize position
    set(gcf, 'PaperPositionMode', 'auto'); %set fig size to match displayed size (i.e. maximized)
