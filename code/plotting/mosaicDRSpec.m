...
clLens = [DRevents.stop]-[DRevents.start]; %length of each clip (syllable)
cumLens = cumsum(clLens);
if ~isinf(params.maxMosaicLength) && cumLens(end) > params.maxMosaicLength %if length of all syllables (in this cluster) is > params.maxMosaicLength
    cutoff = find(cumLens < params.maxMosaicLength, 1, 'last'); %cut off at maxMosaicLength
