...
[juvSylls, juvSessions, juvSpectra, juvFeatures] = loadAgeSylls(birdID, age); %basically loading from allSpecs
...
try
    ...
    if strcmp('loadAcceptedLabels:fileNotFound', err.identifier)    %if this specific error was encountered, assign clusterIdxs = []
        clusterIdxs = [];
    else
        rethrow(err);  %otherwise just rethrow whatever error was encountered      
    end
    [juvSylls.type] = deal(NaN); %assign NaN to the 'type' field of juvSylls structure
end
