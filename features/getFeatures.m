...
featSpecs = initEmptyStructArray(params.reduceFeatures, nRegions);
for ii = 1:nRegions
    roi = regions(ii);
    ...
    %tempSpec = getMTSpectrumStats(paddedClip, params.fine); %RY changed to
    %add pitchGoodness & times to features calculated
    pF = params.fine;
    pF.features = [pF.features 'pitchGoodness', 'times'];
    tempSpec = getMTSpectrumStats(paddedClip, pF);
    ...
    %     end
    %RY changed such that spectra structure matched features structure, +
    %all features calculated
    
    for jj = 1:numel(params.reduceFeatures)
        featSpecs(ii).(params.reduceFeatures{jj}) = tempSpec.(params.reduceFeatures{jj});
    end
    regionInContext = adjustTimeStamps(roi, -roi.start);
    features(ii) = extractFeatures(tempSpec, regionInContext);
    
    paddedSpecs = featSpecs;
    if params.plot
        plotAllFigures(paddedSpecs(ii), regionInContext, params);
    end
