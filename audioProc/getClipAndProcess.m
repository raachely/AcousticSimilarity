...
if params.doFilterNoise && length(clip) / fs > minFiltLength
    if isempty(songStruct) && isempty(params.noiseFilter) && isfield(region,'file') && ~contains(region.file, 'tutor') %%RY added last condition. TUT song doesn't have noise mask file
